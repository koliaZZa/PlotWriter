...
public:
	std::string titlename = "Nazvanie_plaisholder";
	std::string xname = "plaisholder_x";
	std::string yname = "plaisholder_y";

	TMultiGraph* multiGraph = new TMultiGraph();
	TCanvas* canvas = new TCanvas("canvas", "", 1200, 800);

	static DrawGraphs SetNames(std::string ititlename, std::string ixname, std::string iyname) {
		DrawGraphs gaph = DrawGraphs();
		gaph.titlename = ititlename;
		gaph.xname = ixname;
		gaph.yname = iyname;
		gaph.multiGraph->SetTitle((ititlename + ";" + ixname + ";" + iyname).c_str());
...
void Run()
{
	...
	std::vector<std::string> quantities{ "Cx", "Cy", "Cz", "mx", "my", "mz", "Cx_1", "Cy_1", "Cz_1", "mx_1", "my_1", "mz_1" };

	Model::PlotCollection allRuns;

	// обход директории
	const fs::path rPath{ "./data/" };
	for (const auto& entry : fs::directory_iterator{ rPath })
	{
		try
		{
			...
			{
				if (entry.path().filename().string()[0] == '1')                                 // отсеять файлы протоколов (TODO улучшить принцип)
					allRuns.OpenRun(entry.path().string(), Model::ProtocolTypeEnum::T128);
			}
		}
		catch (const std::exception& e)
		{
			std::cout << "Error accessing " + entry.path().string() + ": " + e.what();
		}
	}

	allRuns.ReadChannels("./data/channels.txt");
	allRuns.ReadRunsList("./data/runs list.txt");
	allRuns.ProcessRuns();
...
int main() {
	...
	//DrawGraphs gr = DrawGraphs::SetNames("a", "b", "c");
	
	// gr.AddLine() сколько надо столько надо
